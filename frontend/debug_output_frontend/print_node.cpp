    ...
    if(node->type == OPERATOR){
        ...
        if(node->type == CONST){
            ...
            if(node->var_func_name){
                ...
                fprintf(dot_file, " node_%p[shape=\"Mrecord\", style=\"filled\", fillcolor=\"#DAA520\", rank=%d, color = \"#964B00\", penwidth=1.0, label=\"{{type = VARIABLE} | {val = %d(%s)} | {0 | 0}} \"];\n", node, *rank, node->data.var_code, mtk->var_info[node->data.var_code].variable_name);
            }
            else if(node->data.var_code < mtk->first_free){
                fprintf(dot_file, " node_%p[shape=\"Mrecord\", style=\"filled\", fillcolor=\"#DAA520\", rank=%d, color = \"#964B00\", penwidth=1.0, label=\"{{type = VARIABLE} | {val = %d} | {0 | 0}} \"];\n", node, *rank, node->data.var_code);
            }
            else{
