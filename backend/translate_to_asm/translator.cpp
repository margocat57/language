...
static void CreateAsmCodeRecursive(FILE *file, TreeNode_t *node, metki* mtk, TreeErr_t* err);
...
static void CreateAsmCodeRecursive(FILE *file, TreeNode_t *node, metki* mtk, TreeErr_t* err){
    ...
    switch(node->type){
        ...
        case FUNCTION:      size_t metka_func = MetkiAddName(mtk, node->var_func_name);
        ...
        case CONST:         fprintf(file, "PUSH %d\n", node->data.const_value); 
                            break;

        case VARIABLE:  fprintf(file, "PUSHR RBX\n"); 
                        fprintf(file, "PUSHR %d\n", node->data.var_code); 
                        fprintf(file, "ADD\n");
        ...
        case OPERATOR:{
            switch(node->data.op){
                case OP_COMMA: 
                    ...
                    fprintf(file, "PUSH %d\n", ff); 
                    fprintf(file, "ADD\n");
                    fprintf(file, "PUSH %d\n", node->left->data.var_code); 
                    fprintf(file, "ADD\n");
                    fprintf(file, "POPR RСX\n");
                    fprintf(file, "POPM [СX]\n");
                    CreateAsmCodeRecursive(file, node->right, mtk, err);
                case OP_RETURN: 
                ...
                case OP_ELSE:
                    ...
                    snprintf(if_buffer, 299, "else_%d", else_count);
                    else_count++;
