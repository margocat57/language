...
#include "translate_to_asm/translate.h"
...
int main(){
    ...
    // making asm code -----------------------------------
    TreeHead_t* head = MakeBackendTree("make_backend_tree/tree_file/tree.txt");
    TreeErr_t err = NO_MISTAKE;
    CreateAsmCode("Processor-and-assembler/assembler_task/expr.txt", head, &err);
    if(err) return 0;

    /*
    // making bytecode ------------------------------------
    assembler assembl = asm_init("Processor-and-assembler/assembler_task/expr.txt");
    if (parser(&assembl)){
        free_asm(&assembl);
        return 0;
    }
    asm_dump(&assembl);
    put_buffer_to_file("Processor-and-assembler/assembler_task/ass.bin", &assembl);
    free_asm(&assembl);

    // running bytecode ----------------------------------
    processor baikal = {};
    baikal = init("Processor-and-assembler/assembler_task/ass.bin");
